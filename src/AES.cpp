...
//参考资料:https://zh.wikipedia.org/wiki/%E5%88%86%E7%BB%84%E5%AF%86%E7%A0%81%E5%B7%A5%E4%BD%9C%E6%A8%A1%E5%BC%8F#%E5%B8%B8%E7%94%A8%E6%A8%A1%E5%BC%8F
//参考资料:https://www.cryptopp.com/wiki/Advanced_Encryption_Standard
//参考资料:https://github.com/kokke/tiny-AES-c
...
字节集 EazyLib::AESCryptoHelper::Decrypt(字节集 解密数据, 字节集 密钥, 字节集 向量)
{
	...
	case AESCryptoHelper::AESMode_t::MODE_ECB:
		...
		break;
	case AESCryptoHelper::AESMode_t::MODE_CBC:
		AES_CBC_Decrypt(解密数据, 解密结果);
		break;
	case AESCryptoHelper::AESMode_t::MODE_CTR:
		AES_CTR_Decrypt(解密数据, 解密结果);
		break;
	case AESCryptoHelper::AESMode_t::MODE_CFB:
		AES_CFB_Decrypt(解密数据, 解密结果);
		break;
	case AESCryptoHelper::AESMode_t::MODE_OFB:
		AES_OFB_Decrypt(解密数据, 解密结果);
		break;
	default:
		break;
	}
...
void EazyLib::AESCryptoHelper::AES_CBC_Encrypt(字节集& 加密数据, 字节集& 加密结果)
{

	unsigned char* pBuf = 加密数据.data();
...
void EazyLib::AESCryptoHelper::AES_CBC_Decrypt(字节集& 解密数据, 字节集& 解密结果)
{
	unsigned char storeIv[16];

	unsigned char* pBuf = 解密数据.data();
	for (unsigned int n = 0; n < 解密数据.size() / 16; n++)
	{
...
void EazyLib::AESCryptoHelper::AES_CTR_Encrypt(字节集& 加密数据, 字节集& 加密结果)
{
	unsigned char buffer[16];

	unsigned i;
	int bi;
	for (i = 0, bi = 16; i < 加密数据.size(); ++i, ++bi)
	{
		...
		加密数据[i] = (加密数据[i] ^ buffer[bi]);
	}

	加密结果 = 加密数据;
}

void EazyLib::AESCryptoHelper::AES_CFB_Encrypt(字节集& 加密数据, 字节集& 加密结果)
{
	...
	unsigned char* Ciphertext = 加密数据.data();
	unsigned int bi = 0;
	for (unsigned int n = 0; n < 加密数据.size(); ++n)
	{
...
void EazyLib::AESCryptoHelper::AES_OFB_Encrypt(字节集& 加密数据, 字节集& 加密结果)
{
	//Initialization vector
	Cipher(Iv);
...
void EazyLib::AESCryptoHelper::AES_OFB_Decrypt(字节集& 解密数据, 字节集& 解密结果)
{
	//Initialization vector
	Cipher(Iv);

	unsigned char* Plaintext = 解密数据.data();
	unsigned int bi = 0;
	for (unsigned int n = 0; n < 解密数据.size(); ++n)
	{
		Plaintext[bi] ^= Iv[bi];
		bi++;
		if (bi == 16)
...
void EazyLib::AESCryptoHelper::AES_CFB_Decrypt(字节集& 解密数据, 字节集& 解密结果)
{
	解密结果.resize(解密数据.size());

	//Initialization vector
	unsigned char* CIv = Iv;
	Cipher(CIv);

	unsigned char* Ciphertext = 解密数据.data();
	unsigned char* Plaintext = 解密结果.data();
...
void EazyLib::AESCryptoHelper::AES_CTR_Decrypt(字节集& 解密数据, 字节集& 解密结果)
{
	unsigned char buffer[16];

	unsigned i;
	int bi;
	for (i = 0, bi = 16; i < 解密数据.size(); ++i, ++bi)
	{
		if (bi == 16) /* we need to regen xor compliment in buffer */
		{
			memcpy(buffer, Iv, 16);
...
bool EazyLib::AESCryptoHelper::AppendPadding(字节集& 数据)
{
	...
	case PaddingMode_t::zero_padding:
		appendZeroPadding(数据, 16);
		break;
	case PaddingMode_t::pkcs5_padding:
	case PaddingMode_t::pkcs7_padding:
		//对于AES算法来说,pkcs5与pkcs7无区别
		appendPkcs7Padding(数据, 16);
		break;
	default:
		break;
	}
...
void EazyLib::AESCryptoHelper::RemovePadding(字节集& 数据)
{
	switch (m_PaddingMode)
	{
	case PaddingMode_t::no_padding:
		break;
	case PaddingMode_t::zero_padding:
		removeZeroPadding(数据, 16);
		break;
	case PaddingMode_t::pkcs5_padding:
	case PaddingMode_t::pkcs7_padding:
		removePkcs7Padding(数据, 16);
		break;
	default:
		break;
	}
