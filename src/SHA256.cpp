...
inline void _bswapw(unsigned int* p, unsigned int i)
{
	while (i--) p[i] = (RR(p[i], 24) & 0x00ff00ff) | (RR(p[i], 8) & 0xff00ff00);

} /* _bswapw */

inline void _rtrf(unsigned int* b, unsigned int* p, unsigned int i, unsigned int j)
{
...
void sha256_hash(sha256_context* ctx, unsigned char* dat, unsigned int sz)
{
	register unsigned int i = ctx->len[0] & 63, l, j;
...
void sha256_done(sha256_context* ctx, unsigned char* buf)
{
...
字节集 EazyLib::哈希_SHA256(字节集 数据)
{
	sha256_context sha256Context;
	sha256_init(&sha256Context);
	sha256_hash(&sha256Context,数据.data(),数据.size());

	字节集 ret;
	ret.resize(0x20, 0x0);
	sha256_done(&sha256Context, ret.data());
	return ret;
}